...
#Para Lasso solo usar valores positivos.  
# λ = 0.1 
lasso_model_cv <- train(x_train, y_train,
                     method = "glmnet",
                     trControl = train_control,
                     tuneGrid = expand.grid(alpha = 1,   # alpha = 0 para Ridge
                                            lambda = 0.1)) # Regularización de Ridge con λ = 0.1

#coef(lasso_model_cv$finalModel, s = lasso_model_cv$bestTune$lambda)


# Evaluamos el modelo en train and test
# Hacer predicciones
predicciones <- predict(lasso_model_cv, newdata = x_test)
...
# Hacer predicciones
predicciones_train <- predict(lasso_model_cv, newdata = x_train)
