...
dir_maiac <- "D:/Josefina/Proyectos/ProyectoChile/modelos/dataset_ejemplo/Prediccion_01-2024/00_MAIAC_2024_B"
...
dir_era <- "D:/Josefina/Proyectos/ProyectoChile/modelos/dataset_ejemplo/Prediccion_01-2024/05_ERA5/05_ERA5_2024/"
...
dir_merra <- "D:/Josefina/Proyectos/ProyectoChile/modelos/dataset_ejemplo/Prediccion_01-2024/04_MERRA-2_Dia/2024/"
...
setwd("D:/Josefina/Proyectos/ProyectoChile/modelos/dataset_ejemplo/Prediccion_2015/tiff/")
dir_salida <- "D:/Josefina/Proyectos/ProyectoChile/modelos/dataset_ejemplo/Prediccion_2015/Salida/Salida_03-XGB_cv_M1-041024/"
# Fechas de inter?s
fechaInicio <- as.Date("01-01-2015", format = "%d-%m-%Y")
fechaFin <- as.Date("31-01-2015", format = "%d-%m-%Y")
...
for (j in 1:nrow(lista_fecha)) {
  ...
  fechaNDVI<- as.Date("01-01-2015", format = "%d-%m-%Y")
  ...
  #plot(NDVI_raster)
  num_na_NDVI <- sum(is.na(NDVI_raster[[1]]))
  print("NDVI",num_na_NDVI)
  ################# -----     LandCover     -----
  ...
  LandCover_NA <- sum(is.na(LandCover_raster[[1]]))
  print("landCover",LandCover_NA)
  ...
  #plot(DEM_raster)
  DEM_raster_NA <- sum(is.na(DEM_raster[[1]]))
  print("DEM",DEM_raster_NA)
  ################# -----     MERRA-2     -----
  ...
  BCSMASS_raster_NA <- sum(is.na(BCSMASS_raster[[1]]))
  print("BCSMASS",BCSMASS_raster_NA)
  #plot(BCSMASS_raster)
  ...
  DMSSMASS_raster_NA <- sum(is.na(DMSSMASS_raster[[1]]))
  print("DMSSMASS",DMSSMASS_raster_NA)
  ...
  DUSMASS_raster_NA <- sum(is.na(DUSMASS_raster[[1]]))
  print("DUSMASS",DUSMASS_raster_NA)
  #plot(DUSMASS_raster)
  ...
  DUSMASS25_raster_NA <- sum(is.na(DUSMASS25_raster[[1]]))
  print("DUSMASS25",DUSMASS25_raster_NA)
  ...
  OCSMASS_raster_NA <- sum(is.na(OCSMASS_raster[[1]]))
  print("OCSMASS",OCSMASS_raster_NA)
  #plot(OCSMASS_raster)
  ...
  SO2SMASS_raster_NA <- sum(is.na(SO2SMASS_raster[[1]]))
  print("SO2SMASS",SO2SMASS_raster_NA)
  ...
  SO4SMASS_raster_NA <- sum(is.na(SO4SMASS_raster[[1]]))
  print("SO4SMASS",SO4SMASS_raster_NA)
  ...
  SSSMASS_raster_NA <- sum(is.na(SSSMASS_raster[[1]]))
  print("SSSMASS",SSSMASS_raster_NA)
  #plot(SSSMASS_raster)
  ...
  SSSMASS25_raster_NA <- sum(is.na(SSSMASS25_raster[[1]]))
  print("SSSMASS25",SSSMASS25_raster_NA)
  ...
  #plot(BLH_raster)
  BLH_raster_raster_NA <- sum(is.na(BLH_raster[[1]]))
  print("BLH",BLH_raster_raster_NA)
  ## D2M
  D2M_raster <- raster(paste("05_ERA5/",fechaInteres,"-D2M_raster.tif",sep=""))
  #plot(D2M_raster)
  D2M_raster_NA <- sum(is.na(D2M_raster[[1]]))
  print("D2M",D2M_raster_NA)
  ## T2M
  T2M_raster <- raster(paste("05_ERA5/",fechaInteres,"-T2M_raster.tif",sep=""))
  #plot(T2M_raster)
  T2M_raster_NA <- sum(is.na(T2M_raster[[1]]))
  print("T2M",T2M_raster_NA)
  
  ## TP
  TP_raster <- raster(paste("05_ERA5/",fechaInteres,"-TP_raster.tif",sep=""))
  #plot(TP_raster)
  TP_raster_NA <- sum(is.na(TP_raster[[1]]))
  print("TP",TP_raster_NA)
  ## SP
  SP_raster <- raster(paste("05_ERA5/",fechaInteres,"-SP_raster.tif",sep=""))
  #plot(SP_raster)
  SP_raster_NA <- sum(is.na(SP_raster[[1]]))
  print("SP",SP_raster)
  ## V10
  V10_raster <- raster(paste("05_ERA5/",fechaInteres,"-V10_raster.tif",sep=""))
  #plot(V10_raster)
  V10_raster_NA <- sum(is.na(V10_raster[[1]]))
  print("V10",V10_raster_NA)
  ## U10
  U10_raster <- raster(paste("05_ERA5/",fechaInteres,"-U10_raster.tif",sep=""))
  #plot(U10_raster)
  U10_raster_NA <- sum(is.na(U10_raster[[1]]))
  print("U10",U10_raster_NA)
  ################# -----     DayWeek     -----
  ################# -----     ERA5     -----
  ## BLH     -----
  dayWeek_raster <- raster(paste("06_weekDay/",fechaInteres,"-weekDay_raster.tif",sep=""))
  #plot(dayWeek_raster)
  dayWeek_raster_NA <- sum(is.na(dayWeek_raster[[1]]))
  print("dayWeek",dayWeek_raster_NA)
  ##### STACK
  ...
  names(r_stack_df) <- c( "AOD_055" ,"ndvi","LandCover","BCSMASS_dia",
...
View(df_rbind)
write.csv(df_rbind, paste(dir_salida,"salida_02-XGB_cv_M4-300924_01-2024_B.csv",sep=""))
...
data_merge <- prediccion_2024 %>%
  left_join(data_estaciones_2024, by = c("date", "estacion")) #%>%  # Merge basado en dos columnas
...
data_pm <- data_merge[complete.cases(data_merge$validados),]
...
modelo <- lm(valor_raster ~validados , data = data_pm)
# Calcular R?
...
bias <- mean(data_pm$valor_raster - data_pm$validados)

# pearson <- cor(data_pm$valor_sinca, data_pm$X02_RF.CV.M1.090924, method = "pearson")
pearson <- cor(data_pm$validados, data_pm$valor_raster, method = "pearson")
# Crear scatter plot con l?nea de regresi?n
#ggplot(data_pm, aes(x = valor_sinca, y = X02_RF.CV.M1.090924)) +
ggplot(data_pm, aes(x = validados, y = valor_raster)) +
  geom_point(color = "#99d8c9", ,alpha= 0.7,size = 2) +  # Puntos del scatter plot
  geom_smooth(method = "lm", color = "red", se = FALSE) +  # L?nea de regresi?n lineal
  labs(x = "SINCA", y = "Prediccion", title = "Prediccion 01-06 2024 02-XGB_cv_M4-300924") +
  theme_classic() +
  ...
  geom_text(aes(x = 4.3, y = 85,
                label = paste("r = ",round(pearson,2))),
            stat = "unique",
            size = 2.5, color = "Black")+

  geom_text(aes(x = 6, y = 80,
                label = paste("RMSE = ",round(rmse,2))),
            stat = "unique",
            size = 2.5, color = "Black") +

  
  geom_text(aes(x = 5.2, y = 75,
