...
for (i in 1:1){
  # datos estacion PM
  data_PM <- read.csv(paste("D:/Josefina/Proyectos/ProyectoChile/dataset/proceed/estaciones_media_dia/",num_estacion,"-",estacion,"_PM25.csv",sep=""))
  #data_PM$date  <- strptime(data_PM$fecha, format = "%Y-%m-%d %H:%M:%S")
  ...
  #Guardamos
  name <- paste("D:/Josefina/Proyectos/ProyectoChile/dataset/proceed/merge_pm_maiac/",ventana,"/",num_estacion,"-",estacion,"_merge_PM25-MAIAC_dailyMean.csv",sep="")
  print(length(merge_tot_subt_complete$AOD_550))
  write.csv(merge_tot_subt_complete,name)
}

name <- read.csv("D:/Josefina/Proyectos/ProyectoChile/dataset/proceed/merge_pm_maiac/1000/09-TOT_merge_PM25-MAIAC_dailyMean.csv")
name$date  <- strptime(name$date, format = "%d/%m/%Y")
write.csv(name,"D:/Josefina/Proyectos/ProyectoChile/dataset/proceed/merge_pm_maiac/1000/09-TOT_merge_PM25-MAIAC_dailyMean.csv")
